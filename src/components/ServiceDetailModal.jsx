...
const ServiceDetailModal = ({ service, onClose }) => {
  ...
  const amenityDetails = service.amenities?.map(amenityId =>
  ...
  return createPortal(
    <div className="service-modal" role="dialog" aria-modal="true" aria-labelledby="service-modal-title">
      ...
      <div className="service-modal-content" role="document">
        <button type="button" className="service-modal-close" onClick={onClose} aria-label="Cerrar detalle de servicio">
          ✕
        </button>
        ...
        <div className="service-modal-body">
          ...
          <div className="service-modal-info">
            <header className="service-modal-header">
              ...
              <p className="service-modal-subtitle">{service.shortDescription}</p>
            </header>
            ...
            {amenityDetails.length > 0 && (
              <section className="service-modal-section" aria-label="Amenidades incluidas">
                <h3>Amenidades</h3>
                <ul className="service-modal-amenities">
                  {amenityDetails.map(detail => (
                    <li key={detail.id}>
                      <span className="amenity-label">{detail.name}</span>
                      <span className="amenity-note">{detail.description}</span>
                    </li>
                  ))}
                </ul>
              </section>
            ...
            {service.extras && service.extras.length > 0 && (
              <section className="service-modal-section" aria-label="Extras disponibles">
            ...
            {service.requirements && service.requirements.length > 0 && (
              <section className="service-modal-section" aria-label="Recomendaciones previas">
        ...
        <footer className="service-modal-footer">
          ...
          <Link className="btn btn-outline" to={`/servicios/${service.slug}`} onClick={onClose}>
            Ver ficha completa
          </Link>
        </footer>
      </div>
